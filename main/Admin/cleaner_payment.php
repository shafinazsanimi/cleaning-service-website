...
<div class="grid">
		...
		<div class="row">
			...
			<div class="c3">
				<div class="leftsidebar">
					<h2 class="title stresstitle">Payment Menu</h2>
					<hr class="hrtitle">
					...
					<?php
						...
						$result = mysqli_query($con, "select payment_username from payment_cl ");
					while($row=mysqli_fetch_assoc($result))	
					{
					?>
					<?php echo "Cleaner ID : " . $row["payment_username"]; ?>
					...
					payment_cl.payment_id		
					FROM 
					payment_cl  
					RIGHT JOIN 
					cleaner 
					ON
					cleaner.cleaner_username = payment_cl.payment_username AND cleaner.cleaner_username='$id'

					GROUP BY payment_cl.payment_id
					");
					...
					while($row=mysqli_fetch_assoc($result))	
					{
					  ...
					  echo "Cleaner Payment : " . $row["payment_id"] . "      " ; 
		...
		<table>
		<tr>
		<th><center>Input Cleaner ID To List Out Payment ID
		<th><center>Input Payment ID to Check out Payment
	</tr>
	...
	$result = mysqli_query($con, "select * from payment_cl where payment_id ='$io'");
	while($row=mysqli_fetch_assoc($result))
	{
	?>
		...
		<?php echo " User Name : " . $row["payment_username"]; ?>
	
	
		<?php echo " Amount : " . $row["payment_amount"]; ?>
	
	
		<?php echo " Account No. : " . $row["payment_account"]; ?>
